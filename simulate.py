...
def create_report(num_cust, strategy, segments):
    """Gets costs and c reates new entry for simulation results"""
    trips = [scenario, num_cust, strategy, 'trip count', get_trip_count(segments)]
    radial = [scenario, num_cust, strategy, 'radial cost', sum([get_radial_cost(inst, seg) for seg in segments])]
    circular = [scenario, num_cust, strategy, 'circular cost', sum([get_circular_cost(inst, seg) for seg in segments])]
    total = [scenario, num_cust, strategy, 'total cost', sum([get_total_cost(inst, seg) for seg in segments])]
        ...
        for i in range(cust_sims):
            for j in range(dem_sims):
                ...
                try:
                    ...
                    sim_results = sim_results.append(create_report(num_cust, 'dedicated', segments), ignore_index=True)
                    dt += time.time() - new_dt
                    ...
                    segments = implement_k_overlapped_alg(inst, primary_routes, extended_routes, capacity, route_size,
                                                          overlap_size)
                    sim_results = sim_results.append(create_report(num_cust, 'overlapped', segments), ignore_index=True)
                    ot += time.time() - new_ot

                    # Solve fully flexible routing
                    new_ft = time.time()
                    segments = create_full_trips(inst, [inst.tour[1:]], capacity)
                    sim_results = sim_results.append(create_report(num_cust, 'fully flexible', segments),
                                                     ignore_index=True)
                    ft += time.time() - new_ft
                    ...
                    new_rows = pd.DataFrame([[scenario, num_cust, 'reoptimization', 'total cost', cost],
                                            ...
                                            columns=['Scenario', 'Number of Customers', 'Routing Strategy', 'Metric',
                                                     'Value'])
...
if __name__ == "__main__":
    ...
    baseline_sim = simulate(scenario = 'baseline', problem_sizes = [5, 10, 20], capacity = 20, route_size = 5, overlap_size = 5, cust_sims = 3, dem_sims = 100)
    baseline_sim.head()
